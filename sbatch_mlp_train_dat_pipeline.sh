...
declare -a dgps= ( "full_ddm2" )  #( "weibull_cdf" ) # ( "ddm_seq2_angle" "ddm_mic2_angle" "ddm_par2_angle" )
            ...
            then
                for n_c in "${n_choices[@]}"
                    do
                       ...
                       python -u kde_train_test.py --machine $machine --method $dgp --simfolder training_data_binned_${binned}_nbins_${bins}_n_${n} --fileprefix ${dgp}_nchoices_${n_c}_train_data_binned_${binned}_nbins_${bins}_n_${n} --outfolder training_data_binned_${binned}_nbins_${bins}_n_${n} --nbyparam $nbyparam --mixture 0.8 0.1 0.1 --fileid $SLURM_ARRAY_TASK_ID --nproc $nproc
