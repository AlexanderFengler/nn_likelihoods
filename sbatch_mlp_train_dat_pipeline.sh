...
declare -a dgps=( "ornstein" ) # ( "ddm_seq2_angle" "ddm_mic2_angle" "ddm_par2_angle" ) 
        ...
        do
            ...
            then
                for n_c in "${n_choices[@]}"
                 ...
                 python -u kde_train_test.py --machine $machine --method $dgp --simfolder training_data_binned_${binned}_nbins_${bins}_n_${n} --fileprefix ${dgp}_nchoices_${n_choices[0]}_train_data_binned_${binned}_nbins_${bins}_n_${n} --outfolder training_data_binned_${binned}_nbins_${bins}_n_${n} --nbyparam $nbyparam --mixture 0.8 0.1 0.1 --fileid $SLURM_ARRAY_TASK_ID
            fi
        done
