...
def kde_train_test_from_simulations_flexbound_2(base_simulation_folder = '',
                                                target_folder = '',
                                                idx = 1,
                                                n_total = 10000,
                                                p_train = 0.8,
                                                ...
                                                process_params = ['v', 'a', 'w', 'c1', 'c2'],
                                                print_info = True,
                                                files_to_consider = 'auto'):
    
    # Get files 
    
    if files_to_consider == 'auto':
        files_ = os.listdir(base_simulation_folder)
        files_.sort(key = str.lower)
    else: 
        files_ = files_to_consider
    ...
    # Generate basic empty dataframe 
    my_columns = process_params + ['rt', 'choice', 'log_l']
    data = pd.DataFrame(np.zeros((n_samples_total, len(my_columns))), 
                        columns = my_columns)
    ...
    # Write stuff to file 
    print('write data to folder') 
    data.to_pickle(target_folder + '/data_' + str(idx) + '.pickle', protocol = 4)
    
    print('write misc files to folder')
    pickle.dump(tmp_sim_data,  open(target_folder + '/meta_data.pickle', 'wb') ) 
    pickle.dump(files_to_consider, open(target_folder + '/files_' + str(idx) + '.pickle', 'wb') )
...
def kde_make_train_test_split_2(folder = '',
                                p_train = 0.8):
    ...
    data.iloc[train_id, :(len(my_columns) - 1)].to_pickle(folder + 'train_features.pickle',
                                                          protocol = 4)
    
    data.iloc[test_id, :(len(my_columns) - 1)].to_pickle(folder + 'test_features.pickle', 
                                                         protocol = 4)
    
    data.iloc[train_id, (len(my_columns) - 1)].to_pickle(folder + 'train_labels.pickle', 
                                                         protocol = 4)
    
    data.iloc[test_id, (len(my_columns) - 1)].to_pickle(folder + 'test_labels.pickle',
                                                        protocol = 4)
